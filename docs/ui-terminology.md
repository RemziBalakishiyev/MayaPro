...
| 65 | `src/features/employees/components/SalaryCard.tsx` / `SalaryPayModal.tsx` — işçi kartının əsas əməliyyat düyməsi/modal başlığı | `Pul ver` | `Maaş ödə` | Əməliyyatın nə etdiyi (maaş ödənişi) mətndən birbaşa aydın olsun; düymə eyni zamanda kartın vizual olaraq ən qabarıq (əsas) əməliyyatına çevrildi | FE#79, AC-7 |
| 66 | `src/features/employees/components/SalaryCard.tsx` — 3 standart göstərici etiketləri | `Maaş` / `Verilib` / `Qalıb` (`Artıq verilib`) | `Aylıq maaş` / `Bu ay ödənilib` / `Qalıq məbləğ` (`Artıq ödəniş`) | Qısaldılmış etiketlər əvəzinə tam, birmənalı mətn — hesablama dəyişmədi | FE#79, AC-3/AC-4 |
| 67 | `src/features/employees/components/SalaryCard.tsx` — maaş 0-dırsa "Aylıq maaş" sahəsi | `0.00 ₼` (redaktə edilə bilən rəqəm kimi) | `Maaş təyin olunmayıb` (+ Sahibkar üçün redaktə keçidi) | Sıfır maaş "hələ təyin edilməyib" mənasında aydın göstərilsin, "0 AZN maaş" ilə qarışmasın | FE#79, AC-5 |
| 68 | `src/features/employees/components/SalaryCard.tsx` / `EmployeesTable.tsx` — "Rol" sütunu/nişanı | Backend-in xam kod dəyəri (məs. `sahib`) | `Sahibkar` / `Menecer` / `Kassir` / `Satıcı` (`employeeRoleLabel()`) | Xam kod istifadəçiyə göstərilməsin, hər rol sabit rəngli `Badge` tonunda | FE#79, AC-6 |
| 69 | `src/features/employees/components/SalaryDeductionModal.tsx` — modal başlığı/düyməsi | `Tutulma yaz` / `Təsdiq et` (ad-hoc birbaşa submit) | `Tutulma əlavə et` / `Davam et` → paylaşılan `ConfirmDialog` (`Tutulmanı təsdiqlə`) | `SalaryPayModal`-la eyni iki-addımlı təsdiq naxışına uyğunlaşdırıldı (AC-9) | FE#79, AC-8/AC-9 |
| 70 | `src/features/employees/components/SalaryBoard.tsx` — xəta vəziyyəti | Ad-hoc qırmızı qutu, sabit mətn/`error.message` | Paylaşılan `InlineError`: `Maaş məlumatı yüklənmədi` + texniki detal + "Yenidən" | Digər səhifələrlə (F-44) eyni xəta dili | FE#79, AC-12 |
