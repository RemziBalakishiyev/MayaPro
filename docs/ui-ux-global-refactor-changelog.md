...
| `PeriodFilter.tsx` (`:48,206,262` + tarix input-ları) | tarix giriş sahələri `h-9` (36px) → **`h-10`** (40px), FE#86 qeydi (`PageToolbar` heç vaxt mövcud olmayıb şərhi) | `focus-ring-inset`, `rounded-chip`/`rounded-control` token-ları, `SegmentedDateFilter` alias | Hər ikisi birləşdi: FE#69-un bütün token/alias işi qalıb, YALNIZ tarix giriş sahələrinin hündürlüyü `h-9` → **`h-10`** və FE#86 şərhi `main`-dən götürülüb | `main` tam seçilsəydi → `focus-ring-inset`/alias (AC-9) itərdi. FE#69 tam seçilsəydi → tarix sahələri 36px qalıb **AC-8**-i pozardı, FE#86 şərhi də olmazdı |
